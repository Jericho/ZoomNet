...
namespace ZoomNet.IntegrationTests
{
	internal class TestsRunner
	{
		...
		public async Task<int> RunAsync()
		{
			...
			{
				var clientId = Environment.GetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_CLIENTID", EnvironmentVariableTarget.User);
				var clientSecret = Environment.GetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_CLIENTSECRET", EnvironmentVariableTarget.User);

				var refreshToken = Environment.GetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_REFRESHTOKEN", EnvironmentVariableTarget.User);
				var accessToken = Environment.GetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_ACCESSTOKEN", EnvironmentVariableTarget.User);
				connectionInfo = new OAuthConnectionInfo(clientId, clientSecret, refreshToken, accessToken,
					(newRefreshToken, newAccessToken) =>
					{
						Environment.SetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_REFRESHTOKEN", newRefreshToken, EnvironmentVariableTarget.User);
						Environment.SetEnvironmentVariable("SCHOOLBOARD_ZOOM_OAUTH_ACCESSTOKEN", newAccessToken, EnvironmentVariableTarget.User);
					});

				//var authorizationCode = "2otvW4K6AC_hzN5KkCjREmZ0oe2X2IPzg";
			...
			var userId = Environment.GetEnvironmentVariable("SCHOOLBOARD_ZOOM_USERID", EnvironmentVariableTarget.User);
